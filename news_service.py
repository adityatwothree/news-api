...
class NewsService:
    ...
    async def get_news_by_source(self, source: str, limit: int = 5) -> List[NewsArticle]:
        """Get news articles by source."""
        articles = self.db.query(NewsArticleDB).filter(
            NewsArticleDB.source_name.ilike(f"%{source}%")
        ).order_by(desc(NewsArticleDB.publication_date)).limit(limit).all()
        
        return await self._process_articles(articles)
    
    async def get_news_by_score(self, threshold: float = 0.7, limit: int = 5) -> List[NewsArticle]:
        """Get news articles by relevance score."""
        articles = self.db.query(NewsArticleDB).filter(
            NewsArticleDB.relevance_score >= threshold
        ).order_by(desc(NewsArticleDB.relevance_score)).limit(limit).all()
    ...
    async def get_nearby_news(self, latitude: float, longitude: float, radius_km: float = 10.0, limit: int = 5) -> List[NewsArticle]:
    ...
    async def get_trending_news(self, latitude: float, longitude: float, radius_km: float = 10.0, limit: int = 5) -> TrendingResponse:
        """Get trending news articles based on user interactions."""
        # Get articles within radius
        nearby_articles = await self.get_nearby_news(latitude, longitude, radius_km, limit * 2)
        ...
        for article in nearby_articles:
            score = await self._calculate_trending_score(article.id, latitude, longitude, radius_km)
            if score > 0:
    ...
    async def _calculate_trending_score(self, article_id: str, latitude: float, longitude: float, radius_km: float) -> float:
        ...
        events = self.db.query(UserEventDB).filter(
            and_(
                UserEventDB.article_id == article_id,
                UserEventDB.timestamp >= cutoff_time,
                func.abs(UserEventDB.latitude - latitude) <= 0.5,  # Rough radius check
        ...
        for event in events:
            ...
            event_score = event_weights.get(event.event_type, 1.0) * recency_factor * distance_factor
            score += event_score
    ...
    async def _process_articles(self, articles: List[NewsArticleDB]) -> List[NewsArticle]:
        ...
        for article in articles:
            ...
            if not llm_summary:
                try:
                    llm_summary = await llm_service.summarize_article(article.title, article.description)
                    # Update database with summary
                    article.llm_summary = llm_summary
                    self.db.commit()
                except Exception as e:
                    print(f"Error generating summary for article {article.id}: {e}")
                    llm_summary = article.description[:200] + "..." if len(article.description) > 200 else article.description
            
            # Parse category from JSON
            try:
                category = json.loads(article.category) if isinstance(article.category, str) else article.category
            except:
    ...
    async def process_query(self, query_analysis: QueryAnalysis, limit: int = 5) -> List[NewsArticle]:
