...
from models import (
    NewsQueryRequest, NewsResponse, TrendingQueryRequest, TrendingResponse,
    HealthResponse
)
from database import get_db, create_tables, populate_database_from_json, generate_sample_user_events
...
async def query_news(
    ...
    try:
        ...
        articles = await news_service.process_query(
            query_analysis, 
            request.limit
        )
...
async def get_news_by_score(
    threshold: float = Query(default=0.7, ge=0.0, le=1.0, description="Relevance score threshold"),
    limit: int = Query(default=5, ge=1, le=50, description="Number of articles to return"),
    db: Session = Depends(get_db)
):
...
async def get_nearby_news(
    ...
    try:
        news_service = NewsService(db)
        articles = await news_service.get_nearby_news(latitude, longitude, radius_km, limit)
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving nearby news: {str(e)}")
...
async def get_trending_news(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving trending news: {str(e)}")
...
async def clear_cache():
    ...
    return {"message": "Cache cleared successfully" if success else "Cache clear failed"}
