...
"""
    ...
    def _parse_query_analysis(self, result: str, user_location: Optional[Dict[str, float]] = None) -> QueryAnalysis:
    ...
    def _fallback_query_analysis(self, query: str, user_location: Optional[Dict[str, float]] = None) -> QueryAnalysis:
        """Fallback analysis when LLM fails."""
        query_lower = query.lower()
        
        # Simple keyword-based intent detection
        if any(word in query_lower for word in ['category', 'type', 'sports', 'technology', 'politics', 'business']):
            intent = QueryIntent.CATEGORY
            category = self._extract_category(query_lower)
        elif any(word in query_lower for word in ['from', 'source', 'cnn', 'bbc', 'reuters', 'times']):
            intent = QueryIntent.SOURCE
            source = self._extract_source(query_lower)
        elif any(word in query_lower for word in ['near', 'nearby', 'location', 'around']):
        ...
        return QueryAnalysis(
            ...
            category=self._extract_category(query_lower) if intent == QueryIntent.CATEGORY else None,
            source=self._extract_source(query_lower) if intent == QueryIntent.SOURCE else None
        )
